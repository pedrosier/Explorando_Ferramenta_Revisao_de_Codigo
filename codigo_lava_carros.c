...
int main(void)
{
  ...
  // printf("Olá, mundo! Acentuação funcionando corretamente.\n");

  // VARIÁVEIS
  float prEta = 1,
        prGas = 1, prAdit = 1, qtdEta, qtdGas, qtdAdit, valRec, cAtendido, tFila = 1;
  float etaRestante = 200, gasRestante = 200, aditRestante = 200;
  int fila = 0, relatorio, menu, tam = 0, cont = 1;
  ...
  } while (prEta < 0 || prGas < 0 || prAdit < 0 || tam <= 0);
